...
impl NxN {
        ...
        T: Into<f64> + Copy
    {
        ...
        if cols.len() != cols[0].len() {
            return Err("given vectors do not form an NxN matrix")
        } else {
            let size = cols.len();
    ...
    fn invert_2x2(&mut self) -> Result<(), &'static str> {
        ...
        if det == 0_f64 {
            return Err("matrix is non-invertible! determinant is 0!")
        }
    ...
    fn invert_3x3(&mut self) -> Result<(), &'static str> {
    ...
    fn invert_4x4(&mut self) -> Result<(), &'static str> {
        let m = &self.mat;
    ...
    fn invert_nxn(&mut self) -> Result<(), &'static str> {
        let n = self.size;
            ...
            for r in 0..n {
                if c == r {
                    ...
                    if self.mat[c][c] == 0_f64 { 
                        return Err("division by zero encountered during matrix inversion")
                    }
